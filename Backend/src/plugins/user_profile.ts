...
async function user_profile(fastify: FastifyInstance, opts: FastifyPluginOptions) {
    fastify.get('/api/user/user_profile', async (request, reply) => {
        ...
        try {
            ...
            const user = fastify.db.prepare("SELECT id, username, email, Avatar FROM users WHERE id = ?").get(decoded.sub) as userType | undefined;
            ...
            const tournamentStats = fastify.db.prepare(`
                                    ...
                                    `).get(user.id, user.id, user.id) as t_stats;
